  ...
  <div>
    ...
    <el-form ref="form" :rules="formRules" :model="form" label-width="80px">
      <el-form-item label="关键词" prop="keyWord">
        <el-input v-model="form.keyWord" style="width: 300px" size="small" placeholder="请输入商品关键词" />
        <span>商品标题中必须包含关键词</span>
      </el-form-item>
      <el-form-item label="商品ID" prop="goodsId">
        <el-input v-model="form.goodsId" style="width: 300px" placeholder="请输入商品ID" size="small" />

        <span>商品ID 如: 566081372583</span>
      </el-form-item>
      <el-form-item label="展示价" prop="price">
        <el-input v-model="form.price" style="width: 300px" placeholder="请输入商品搜索展示价" size="small" />
        <span>为了标签更精准请填写正确的展示价</span>
      </el-form-item>

      <div>
        <el-button v-re-click class="btn-theme" type="primary" style="width:300px;margin-left:80px;" @click="handleQr">
          生成超级标签二维码
        </el-button>
      </div>

      <canvas id="canvas2" ref="qrcode" class="qrcode" />
      <div style="width: 300px;margin-left: 80px;text-align: center; margin-top:12px; ">
        <el-button v-if="qrShow" v-re-click type="primary" class="btn-theme" @click="dowloadQr">保存</el-button>
      </div>
...
import { uploadImageByBase64 } from '@/api/shop'
...
export default {
  ...
  methods: {
    ...
    handlAddYwMarkingSuper() {
      ...
      var canvasData = document.getElementById('canvas2')
      var datanum = canvasData.toDataURL()
      //   var that = this
      uploadImageByBase64(datanum).then(response => {
        if (response.code === 0) {
        //   that.qrdata = response.data
        //   data.codeImg = this.qrdata
          data.codeImg = response.data
    ...
    //   data.codeImg = this.qrdata
    //   console.log('data.asdasd', data, 'qewq', this.qrdata)
    //   parseFloat(data.price).toFixed(2)
    //   addYwMarkingSuper(data).then(response => {
    //     if (response.code === 0) {
    //       this.$message({
