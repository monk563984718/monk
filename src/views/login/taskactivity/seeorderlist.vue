  ...
  <div class="order" style="margin:0 auto;">
    <div class="search-list" style="margin-bottom: 10px">
      ...
      <div style="margin:20px;">
        <span class="search-title" style="margin-left:20px;">任务ID：</span>
        <el-input v-model="query.taskId" size="mini" style="width: 80px;margin-right: 26px" />

        <span class="search-title">订单ID：</span>
        <el-input v-model="query.id" size="mini" style="width: 80px;margin-right: 26px" />

        <span class="search-title">接单账户：</span>
        <el-input v-model="query.accountNum" size="mini" style="width: 80px;margin-right: 26px" />
        ...
        <div style="position:relative;display:inline-block;">
          ...
          <img
            ...
            @click="taskTypeClick"
          >
          <el-select v-model="query.taskType" placeholder="请选择" size="mini" style="width: 80px;margin-right: 26px">
            <el-option
        ...
        <div style="position:relative;display:inline-block;margin-right: 26px">
        ...
        <div style="position:relative;display:inline-block;margin-right: 26px">
          ...
          <img
            ...
            style="width: 14px; height: 14px;position:absolute;top:-7px;right:0;"
            :src="require('@/assets/images/shibai.png')"
            fit="fill"
      ...
      <el-row>
        ...
        <!-- <el-col :span="2">
          <el-button size="mini" class="btn-theme" style="margin-left:20px;" @click="handleAllYes">订单列表详情</el-button>

        </el-col> -->
        <el-col :span="2">

          <el-button size="mini" class="btn-theme" style="margin-left:20px;" @click="handlerouter">返回</el-button>
        </el-col>
      </el-row>
    </div>
    <!-- <el-button size="mini" class="btn-theme" style="margin-left:20px;" @click="handlerouter">返回</el-button> -->
    ...
    >
      ...
      <el-table-column label="支付佣金" width="71">
        <template slot-scope="{row}">
          <span>{{ row.realPayCommission }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="195" fixed="right">
        <template slot-scope="{row}">
          <el-button v-if="row.status !== 1" type="text" disabled @click="handleYes(row.id)">确认</el-button>
          <el-button v-if="row.status !== 1" type="text" :disabled="(row.status !== 1 && row.status !== 2)? false : true" @click="handleDetail(row)">详情</el-button>
          <el-button v-if="row.status !== 1" type="text" disabled @click="handleDialog(2,row.id)">改价</el-button>
          <el-button v-if="row.status !== 1 && row.storeTypeId === 1" type="text" disabled @click="handleHr(row.id)">换人</el-button>
          <el-button v-if="row.status === 1" type="text" @click="handledele(row.id)">撤销</el-button>
          ...
          <el-button v-if="row.status !== 1 && row.storeTypeId === 1" type="text" :disabled="(row.status === 3) ? false : true" @click="handleHr(row.id)">换人</el-button>
          <el-button v-if="row.status === 1" type="text" @click="handledele(row.id)">撤销</el-button> -->
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      if (goodsdatas.type === '任务管理') {
        ...
        orderList(this.query).then(response => {
          ...
          console.log('dingdan订单数据', this.tableData)
        ...
        if (localData) {
          ...
          orderList(localData).then(response => {
            this.tableData = response.data.rows
            // console.log('dingdan订单数据', this.tableData)
            this.pagetotal = Math.ceil(response.data.total / this.query.pageSize)
          })
    ...
    // 搜索
    searchdatas() {
      console.log('搜索填写数据', this.timeRange)
      var listData = {}
      ...
      console.log('搜索填写数据', listData)
      // console.log('搜索填写数据', listData, this.timeRange)
...
<style scoped>
    ...
    .table-new >>>th{font-size: 8px;color: white;background-color: #6666FF;padding: 5px 1px;text-align: center;white-space: nowrap;}
    .table-new >>>td{color: #97A8BE;text-align: center;padding: 0}
    .table-new >>>span{font-size: 8px}
