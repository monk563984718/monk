...
export default {
  ...
  data() {
    return {
      ...
      changePayTimeData: false, // 设置付款时间
  ...
  methods: {
    ...
    changepaytime() {
      console.log('付款时间')
      this.form.appointedOperationTime
    },
      ...
      if (data.screenshotDetailIds.length > 0) {
        if (this.arrMinData.length > 0) {
          ...
          if (this.arrMaxData.length > 0) {
            ...
            // console.log('arrMinData,arrMaxDat都无，原始有', this.arrMinData, this.arrMaxData)
            // console.log('arrMinData,arrMaxDat都无，原始有', this.screenshotDetails)
    ...
    // 任务发布时间验证
    taskOutTime() {
      console.log('任务发布时间抽离')
      // changeTimeData: false, // 设置发送单量时间
    },
    // 任务付款时间验证
    taskPayTime(valTime) {
      console.log('任务付款时间验证抽离')
      // changePayTimeData: false, // 设置付款时间
      // 预售付款时间
      var appointedOperationTimeData = this.form.appointedOperationTime
      var appointedOperationTimeData1 = null
      appointedOperationTimeData1 = appointedOperationTimeData.split('-')
      console.log('预售付款时间', appointedOperationTimeData1)
      const nowdate = new Date()
      // const y = nowdate.getFullYear()
      const m = nowdate.getMonth() + 1
      const d = nowdate.getDate()

      // const datey = Number(appointedOperationTimeData1[0])
      const datem = Number(appointedOperationTimeData1[1])
      const dated = Number(appointedOperationTimeData1[2])
      if (m === datem) {
        if (dated < d) {
          console.log('触发')
          this.changePayTimeData = true
        }
      }
    },
    set(val) {
