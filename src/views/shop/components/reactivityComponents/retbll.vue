<template>
  <div class="form" style="position:relative;">
    ...
    <div v-for="item in addewm" v-if="form.searchType===3" :key="item.id" class="form-item" style="position: relative">
      <span class="form-required">*必填</span>
...
export default {
  ...
  methods: {
    ...
    handleDearchType(val) {
      ...
      this.order2 = []
      this.order3 = []
      // this.form.taskCommentList.keyword = ''
      if (val === 1) {
      ...
      // if (val === 1) {
      //   this.form.taskCommentList = [{ type: this.form.commentType, keyword: '' }]
      // } else if (val === 2) {
      //   this.form.taskCommentList = [{ type: this.form.commentType, keyword: '' }]
      // } else if (val === 3) {
      //   this.form.taskCommentList = [{ type: this.form.commentType, qrCodeImgUrl: '' }]
      // }
    },
    ...
    handleNext() {
      ...
      for (let i = 0; i < this.form.taskCommentList.length; i++) {
        ...
        } else if (this.order2[i] > 0) {
          for (let j = 0; j < this.order2[i]; j++) {
            taskCommentList.push(JSON.parse(JSON.stringify(this.form.taskCommentList[i])))
          }
        } else if (this.order3[i] > 0) {
      ...
      //   for (var otherKeylist = 0; otherKeylist < this.form.taskCommentList.length; otherKeylist++) {
      //     otherKey.push({ keyword: this.form.taskCommentList[otherKeylist].keyword, type: this.form.taskCommentList[otherKeylist].type })
      //   }
      // } else if (this.form.searchType === 2) {
      //   for (var otherKeylisttp = 0; otherKeylisttp < this.form.taskCommentList.length; otherKeylisttp++) {
      //     otherKey.push({
      //       keyword: this.form.taskCommentList[otherKeylisttp].keyword,
      //       type: this.form.taskCommentList[otherKeylisttp].type
      //     })
      //   }
      // } else if (this.form.searchType === 3) {
      //   for (var otherKeylistsp = 0; otherKeylistsp < this.form.taskCommentList.length; otherKeylistsp++) {
      //     otherKey.push({
      //       qrCodeImgUrl: this.form.taskCommentList[otherKeylistsp].qrCodeImgUrl,
      //       type: this.form.taskCommentList[otherKeylistsp].type
      //     })
      //   }
      // }
