<template>
  <div class="deposit-page" style="margin-top:24px;">
    <ul class="attention">
      ...
      <li class="li">禁止使用财付通转账，禁止通过银行柜台、ATM机转账</li>
      <li class="li">要求先转账再提交转账信息</li>
      <li class="li">提交转账信息前仔细核对转账金额、姓名及银行卡号，禁止重复提交转账信息如发现恶意重复提交，将罚款甚至封号</li>
      <li class="li">及时提交转账信息，未及时提交者（超过一天）联系充值客服说明充值时间</li>
    </ul>
    <div class="stepC">
      <div class="item">
        <div class="box">
...
export default {
  ...
  methods: {
    init() {
      var localData = localStorage.getItem('deposit')
      if (localData) {
        localData = JSON.parse(localData)
        this.addForm.bankName = localData.bankName
        this.addForm.cardCode = localData.cardCode
        this.addForm.money = localData.money
        this.addForm.realName = localData.realName
      }
    ...
    getAuthorization: function(options, callback) {
      ...
      var uuidNum = this.$uuid.uuid(16, 32)
      var key = options.Key + uuidNum || uuidNum
    ...
    submit() {
      ...
      var Smith = JSON.stringify(data)
      localStorage.setItem('deposit', Smith)
